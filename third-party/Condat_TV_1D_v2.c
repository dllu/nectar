...
/* double is recommended; in that case the precision is of order 1e-11. With
 * float, the precision for a typical signal with values in [0,255] is of order
 * 5e-3 only. */
...
The algorithm can operate in place, with output=input; in that case the input is
replaced by the output.

See the Matlab code on my webpage for comments.
*/
void TV1D_denoise_v2(datatype* input, datatype* output, unsigned int width,
                     const datatype lambda) {
    unsigned int* indstart_low =
        (unsigned int*)malloc(sizeof *indstart_low * width);
    unsigned int* indstart_up =
        (unsigned int*)malloc(sizeof *indstart_up * width);
    unsigned int j_low = 0, j_up = 0, jseg = 0, indjseg = 0, i = 1, indjseg2,
                 ind;
    ...
    for (; i < width; i++) {
        if (input[i] >= output_low_curr) {
            ...
            output_low_curr +=
                (input[i] - output_low_curr) / (i - indstart_low[j_low] + 1);
            output[indjseg] = output_low_first;
            while ((j_low > jseg) &&
